...
# cd C:\Users\KyraEvers\PycharmProjects\MA_thesis\output
...
for file_nr, filename in enumerate(rawdata_bold_nifti_files):
    ...
    scan = nib.load(pathlib.Path(rawdata_bold_nifti_files[file_nr].path).as_posix())  # .get_fdata() # Load resting-state scan
    # nib.load(pathlib.Path("D:\\Data\\thesis\\rawdata\sub-001\ses-LSD\\func\sub-001_ses-LSD_task-rest_run-01_bold.nii").as_posix())
...
for file_nr, filename in enumerate(rawdata_bold_nifti_files):
    ...
    dict_ent['pipeline'] = 'preproc-rois'
    scan = nib.load(
        pathlib.Path(rawdata_bold_nifti_files[file_nr].path).as_posix())  # .get_fdata() # Load resting-state scan
    # nib.load(pathlib.Path("D:\\Data\\thesis\\rawdata\sub-001\ses-LSD\\func\sub-001_ses-LSD_task-rest_run-01_bold.nii").as_posix())
    ...
    if ((concat_runs) & (dict_ent['run'] == "01")):
        ...
        if check_stationarity:
            ...
            df_flat_list = pd.DataFrame(
                flat_list)  # In format [nr_ROIs, nr_PCs * 2] because there are two entries for every PC, the python of the conclusion and the conclusion in words
    ...
    if save_md_PCs:
        ...
        if dict_ent['run'] == '01':
            dict_ent["concat"] = "run-03_concat"
            # Standardized timeseries
            filepaths_md_PCs = processing.save_md_PCs(dict_ent, layout_der, vars.pattern_derivatives_output, mask_coords_dict,
                                   Schaefer_ROIs_df, TR,
            ...
            print("filepaths_md_PCs: {}".format(filepaths_md_PCs))
