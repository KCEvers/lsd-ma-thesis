...
def compute_entropies(dict_ent, layout_der, pattern_derivatives_output, Schaefer_ROIs_df, nr_PCs, mask_unstand_or_stand, raw_or_PC_unstand_or_stand, theiler_window=0, template_length_SampEn=3, r_fraction_SampEn=0.2, template_length_PermEn=4, tau_PermEn=1):
    ...
    PermEns = compute_PermEn(df, template_length=template_length_PermEn, tau=tau_PermEn).T.reset_index().T.to_numpy().tolist()
...
def EH_SampEn(df_col, r_fraction=.2, template_length=3,
    ...
    if norm:
        ...
        SampleEntropy = SampleEntropy * (1 / (- np.log(1 / N))) # Normalize by maximum entropy (from R function casnet::inf_SampEn)
