...
#
# visualisation.c2_d2_h2_estimate_plot=c2_d2_h2_estimate_plot
# visualisation.d2_or_h2_regression_plot=d2_or_h2_regression_plot

def c2_d2_h2_estimate_plot(dict_ent, layout_der, pattern_derivatives_output, Schaefer_ROIs_df, nr_PCs, mask_unstand_or_stand, raw_or_PC_unstand_or_stand, theiler_window=0, tau = 1, emb_dim=1, max_slope=1, max_residuals=.1, min_rsquared=.4, theiler_estimate_from_space_time_traj=True
    ...
    filepath_c2_d2_h2_estimate_fig = pathlib.Path(
        layout_der.build_path({**dict_ent,
                               ...
                               'type_of_fig': 'd2',
                               'extension':'.png',
                               'suffix': "{}_{}".format(dict_ent['suffix'], 'd2-h2-estimate')},
                              pattern_derivatives_output, validate=False,
                              absolute_paths=True))


    # Get parameters from input file if they exist, otherwise use defaults. First read .json file:
    input_path_json = pathlib.Path(
        layout_der.build_path({**dict_ent, 'pipeline': 'preproc-rois', 'extension': '.json'}, pattern_derivatives_output, validate=False,
                              absolute_paths=True)).as_posix()
    with open(input_path_json, 'r') as file:
        input_dict = json.load(file)  # Read .json file

    # Now check for theiler_window, tau, and emb_dim
    nr_ROIs = Schaefer_ROIs_df.shape[0]
    theiler_window = correct_format(input_dict["theiler"]) if "theiler" in input_dict.keys() else theiler_window
    tau = correct_format(input_dict["tau"]) if "tau" in input_dict.keys() else tau
    emb_dim = correct_format(input_dict["emb_dim"]) if "emb_dim" in input_dict.keys() else emb_dim

    # Use theiler window estimate from space-time trajectory
    if theiler_estimate_from_space_time_traj:
        # Build file path to theiler window estimate
        filepath_theiler_estimate = pathlib.Path(
            layout_der.build_path({**dict_ent,
                                   'pipeline': 'timeseries_analysis',
                                   'suffix': "{}_{}_{}".format(dict_ent['suffix'], 'space-time-traj',
                                                               'theiler-estimate'),
    ...
    # Plotly
    # Do same plot in plotly for consistency's sake
    # subplot_titles = [r'Correlation sums $C_2^m(\epsilon)$',
    #                   r'Correlation dimension $D_2^m(\epsilon)\approx$ {} (scaling region: [{:.2f}, {:.2f}])'.format(
    #                       d2_estimate, d2_eps_scaling_region_min, d2_eps_scaling_region_max),
    #                   r'Correlation entropy $h_2^m(\epsilon) \approx$ {} (scaling region: [{:.2f}, {:.2f}])'.format(
    #                       h2_estimate, h2_eps_scaling_region_min, h2_eps_scaling_region_max)
    #                   ]
    # subplot_titles = [r'Correlation sums $C_2^m(\epsilon)$',
    #                   r'Correlation dimension $D_2^m(\epsilon)\approx$ {} (scaling region: [{:.2f}, {:.2f}])'.format(
    #                       d2_estimate, d2_eps_scaling_region_min, d2_eps_scaling_region_max),
    #                   'Correlation entropy $h_2^m(\epsilon) \\approx$' + '{:.2f} (scaling region: [{:.2f}, {:.2f}])'.format(
    #                       h2_estimate, h2_eps_scaling_region_min, h2_eps_scaling_region_max)
    #                   ]
    # subplot_titles = [r'$C_2^m(\epsilon)$',
    #                   r'$D_2^m(\epsilon)\approx$ {} (scaling region: [{:.2f}, {:.2f}])'.format(
    #                       d2_estimate, d2_eps_scaling_region_min, d2_eps_scaling_region_max),
    #                   # 'h<sub>2</sub><sup>m</sup>(&epsilon) &cong {:.2f} (scaling region: [{:.2f}, {:.2f}])'.format(
    #                   #     h2_estimate, h2_eps_scaling_region_min, h2_eps_scaling_region_max)
    #                   # "$h_2^m(\epsilon)\\approx{0:.2f} \\times 10^{{{1}}}$".format(h2_eps_scaling_region_max, '+20')
    #                   # "$h_2^m(\epsilon)\\approx{0:.2f} \\text{(scaling region: [{{{1:.2f}}}, {{{2:.2f}}})])$".format(h2_estimate, h2_eps_scaling_region_min, h2_eps_scaling_region_max)
    #                   "$h_2^m(\epsilon)\\approx{0:.2f} \\text{{ (scaling region [{1:.2f},{2:.2f}])}}$".format(h2_estimate, h2_eps_scaling_region_min, h2_eps_scaling_region_max)
    #                   # 'h<sub>2</sub><sup>m</sup>(&epsilon) &cong {:.2f} (scaling region: [{:.2f}, {:.2f}])'.format(
    #                   #     h2_estimate, h2_eps_scaling_region_min, h2_eps_scaling_region_max)
    #                   ]
    ...
    for annotation_nr in range(len(fig_c2_d2_h2_estimate['layout']['annotations'])):
        if "Correlation" in annotation['text']:
...
def d2_or_h2_regression_plot(dict_ent, layout_der, pattern_derivatives_output, Schaefer_ROIs_df, nr_PCs, mask_unstand_or_stand, raw_or_PC_unstand_or_stand, d2_or_h2 = "d2", theiler_window=0, tau = 1, emb_dim=1, max_slope=1, max_residuals=.1, min_rsquared=.4
    ...
    filepath_d2_or_h2_regression_fig = pathlib.Path(
        layout_der.build_path({**dict_ent,
                               ...
                               'type_of_fig': 'd2',
                               'extension':'.png',
                               'suffix': "{}_{}-{}".format(dict_ent['suffix'], d2_or_h2, 'regression-per-emb-dim')},
                              pattern_derivatives_output, validate=False,
                              absolute_paths=True))
    ...
    for emb_nr in np.unique(df_select["emb_nr"]):
        ...
        # Plot complete d2
        axs[int(emb_nr)].semilogx(df_emb["epsilon"], df_emb[d2_or_h2],
        ...
        axs[int(emb_nr)].semilogx(df_scaling_emb["epsilon"], df_scaling_emb[d2_or_h2],
                     color=vars.fig_d2_color_inside_scaling_region,
                                  label="Inside scaling region")

        # Plot regression line
        axs[int(emb_nr)].scatter(model_dfs_emb["epsilon"], model_dfs_emb["slope"], color="blue", s=5, label="Regression slope")
...
def space_time_traj_plot(dict_ent, layout_der, pattern_derivatives_output,
    ...
    first_peak_idx = pd.DataFrame(first_peak_idx_list, columns = ["perc", "idx", "value"]) # Convert list to dataframe
